...
public class Questions {
	...
	public String sandwichFilling(String sandwich) {
		int first = sandwich.indexOf("");
		//if (sandwich.indexOf("bread") != sandwich.indexOf())
		return "";
	}
	...
	public int amISearch(String sentence) {
		int count = 0;
		int i = sentence.indexOf(" am ");
		
		while(i != -1) {
			count++;
			i = sentence.indexOf(" am ", i+1);
		}
	...
	public boolean compares(String word, int index, char letter) {
		...
		if (index >= word.length()) {
			...
			return wordLowered.charAt(index) == letter;
		}
